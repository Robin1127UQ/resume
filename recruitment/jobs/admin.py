...
class JobAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        obj.creator = request.user     # 当前系统登录用户作为职位创建人
...
def enter_interview_process(modeladmin, request, queryset):
    ...
    for resume in queryset:
        candidate = Candidate()
        # 把 obj 对象中的所有属性拷贝到 candidate 对象中:   __dict__ 表示这个对象的所有属性
...
class ResumeAdmin(admin.ModelAdmin):
    ...
    list_display = ('username', 'applicant', 'city', 'apply_position', 'bachelor_school', 'master_school', 'image_tag',
                    'major', 'created_date')
